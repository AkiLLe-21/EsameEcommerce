...
public class AdministatorClient : IAdministatorClient
{
    ...
    private readonly ILogger<AdministatorClient> _logger;
    ...
    /// <inheritdoc/>
    public async ValueTask<bool> TryCreateTopicsAsync(string topic, short replicationFactor = 1, int numPartitions = 1)
    {
        if (GetMetadata(topic).Topics.Count != 0)
            return false;
