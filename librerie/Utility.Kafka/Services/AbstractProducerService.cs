...
public abstract class AbstractProducerService<TKafkaTopicsOutput>(
    ILogger<AbstractProducerService<TKafkaTopicsOutput>> logger,
    ...
    : IHostedService, IDisposable where TKafkaTopicsOutput : class, IKafkaTopics
{
    /// <summary>
    /// KafkaTopics di riferimento
    /// </summary>
    protected TKafkaTopicsOutput KafkaTopics => optionsTopics.Value;

    /// <summary>
    /// Elenco topic da elaborare
    /// </summary>
    protected IEnumerable<string> Topics => optionsTopics.Value.GetTopics();

    /// <summary>
    /// Numero di secondi attesi per la prima esecuzione del metodo ExecuteTask
    /// </summary>
    protected int DueTime => optionsProducerService.Value.DelaySeconds;

    /// <summary>
    /// Intervallo di tempo in secondi che indica ogni quanto viene eseguito il metodo ExecuteTask
    /// </summary>
    protected int Period => optionsProducerService.Value.IntervalSeconds;
    ...
    /// <summary>
    /// <see cref="Task"/> utilizzato per verificare lo stato di esecuzione del metodo <see cref="AbstractProducerService.ExecuteTaskAsync"/>
    /// </summary>
